...
void master(char* in, char*out)
{
  ...
  if(num_of_bits == 8)
    {
      data_8 = (char*)malloc(volume_size*sizeof(char));
      if(data_8 == 0)	MPI_Abort(MPI_COMM_WORLD,errno);

      error_code = VReadData(data_8, num_of_bits/8, volume_size, fp, &j);
    }
  else if(num_of_bits == 16)
    {
      data_16 = (unsigned short*)malloc(volume_size*sizeof(short));
  ...
  for(n = 1;n<ntasks;n++)
   ...
   scaleData = (char*)malloc(volume_size*sizeof( char));
 ...
 */ 
  ...
  error_code = VWriteHeader(fp, &vh, group, element);  
  error_code = VWriteData(scaleData, 1, volume_size, fp, &j);  
  fclose(fp);
...
void slave()
{
	...
	//	free(chunk_out);
	    ...
	    if(num_of_bits == 8)
	      {
		chunk_data8 = (char *)malloc(chunk_size*sizeof(char));
		if(chunk_data8 == 0) 
		  MPI_Abort(MPI_COMM_WORLD,errno);
		MPI_Recv(chunk_data8,chunk_size,MPI_CHAR,0,WORKTAG, MPI_COMM_WORLD, &status);
	      ...
	      {
		chunk_data16 = (unsigned short *) malloc(chunk_size*sizeof(short));
		if(chunk_data16 == 0) 
		  MPI_Abort(MPI_COMM_WORLD,errno);
