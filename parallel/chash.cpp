...
//  int (*idcmp)(); /* if NULL, item must be VoxelWithValue */
{
  ...
  while (H->elem_size % (sizeof(ChashElem)-HEHS))
    H->elem_size++;
  H->val = (unsigned short (*)(VoxelWithValue*))val;
  H->idcmp = (int (*)(VoxelWithValue* , VoxelWithValue* ))idcmp;
  H->topindex = 0L;
...
int chash_repush(Chash *H, char *v, unsigned short wo)
{
  ...
  if (H->idcmp)
    ...
    {
      vv = HQElem(t).c;
      if (H->idcmp((VoxelWithValue*)v, (VoxelWithValue*)vv) == 0)
        break;
    }
  else
  {
	...
	for (t= H->hh[wo], tt= 0L; t > 0L; tt= t, t= ((VoxelChashElem*)H->q)[t].l)
    {
      ...
      if (((VoxelWithValue*)v)->x==vvv.x && ((VoxelWithValue*)v)->y==vvv.y &&
	      ((VoxelWithValue*)v)->z==vvv.z)
        break;
    }
  }
  if (tt > 0L)
