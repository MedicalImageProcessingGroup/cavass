...
//======================================================================
/**
 * \file   KinematicsInterCanvas.h
 * \brief  KinematicsInterCanvas definition.
 ...
 */
//======================================================================
#ifndef __KinematicsInterCanvas_h
#define __KinematicsInterCanvas_h
...
//class KinematicsInterCanvas : public wxScrolledWindow {
/** \brief KinematicsInterCanvas - the canvas on which images and other things
 *  are drawn (i.e., the drawing area of the window).
 */
class KinematicsInterCanvas : public MainCanvas 
{
  int  mOverallXSize, mOverallYSize, mOverallZSize;
                                         ///< max count of pixels of displayed images in x,y,z  
  bool           m_bKinematicsInterDone;
  int            m_nROIType;
  int            m_nGradType;
...
protected:
  //static const int sSpacing;           ///< space, in pixels, between each slice (on the screen)
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
public:
    KinematicsInterCanvas ( void );
    KinematicsInterCanvas ( wxWindow* parent, MainFrame* parent_frame, wxWindowID id,
...
public:
  ...
  bool   getKinematicsInterDone(void) const; 
  ...
  void   setKinematicsInterDone(bool done)
  {
    m_bKinematicsInterDone = done;
  }
   ...
   void setRefFrame(int nRefFrame)
  {
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  void RunKinematicsInter();
  void CreateDisplayImage(int which);
  float normal(float x, float sigma);

    DECLARE_DYNAMIC_CLASS(KinematicsInterCanvas)
    DECLARE_EVENT_TABLE()
};
