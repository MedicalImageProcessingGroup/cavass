...
//======================================================================
/**
 * \file:  KinematicsIntraCanvas.cpp
 * \brief  KinematicsIntraCanvas class implementation
...
//#include  "KinematicsIntraAlgorithm.h"
#include "auto_register.h"

using namespace std;
//----------------------------------------------------------------------
KinematicsIntraCanvas::KinematicsIntraCanvas ( void )  {  puts("KinematicsIntraCanvas()");  }
//----------------------------------------------------------------------
KinematicsIntraCanvas::KinematicsIntraCanvas ( wxWindow* parent, MainFrame* parent_frame,
...
//    : wxScrolledWindow ( parent, id, pos, size, wxSUNKEN_BORDER )
{
	...
	mFileOrDataCount = 0;
//	m_KinematicsIntraType = KinematicsIntra_GRADIENT2D;
	m_bKinematicsIntraDone = false;	
...
//----------------------------------------------------------------------
KinematicsIntraCanvas::~KinematicsIntraCanvas ( void ) {
    cout << "KinematicsIntraCanvas::~KinematicsIntraCanvas" << endl;
    wxLogMessage( "KinematicsIntraCanvas::~KinematicsIntraCanvas" );
	...
	free( surf );
	surf = NULL;
	KinematicsIntraRelease();	
}
//----------------------------------------------------------------------
void KinematicsIntraCanvas::release ( void ) {
#if 0
    m_scale   = 1.0;
    freeImagesAndBitmaps();
    m_center = m_width = 0;
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::loadData ( char* name,
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::loadFile ( const char* const fn ) 
{
	...
	nInstancesNum = 0;
	Register_Intra(surf, &strOutPut, &nInstancesNum);
	wxLogMessage("done");
	...
	if( strOutPut.GetCount() > 0 )
	{
		...
		if( strOutPut.GetCount() > 2 )
		{
			...
			{
				mListCtrl->InsertItem( j, wxString::Format("") );
	...
	m_bKinematicsIntraDone = true; 
   
    SetCursor( *wxSTANDARD_CURSOR );	
}
//----------------------------------------------------------------------
void KinematicsIntraCanvas::initLUT ( const int which ) {
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::CreateDisplayImage(int which)
{	
}
//----------------------------------------------------------------------
void KinematicsIntraCanvas::reload ( void ) 
{  
  //  Refresh();
}

//----------------------------------------------------------------------
void KinematicsIntraCanvas::mapWindowToData ( int wx, int wy,
                                 int& x, int& y, int& z ) {
#if 0
 ...
 */
void KinematicsIntraCanvas::OnChar ( wxKeyEvent& e ) {
    //cout << "KinematicsIntraCanvas::OnChar" << endl;
    wxLogMessage( "KinematicsIntraCanvas::OnChar" );
    if (e.m_keyCode==' ') {
        if (isLoaded(1)) {
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnMouseMove ( wxMouseEvent& e ) 
{   
   
}
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnRightDown ( wxMouseEvent& e ) 
{
    SetFocus();  //to regain/recapture keypress events	
}
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnRightUp ( wxMouseEvent& e ) 
{
    //if (isLoaded(0)) {
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnMiddleDown ( wxMouseEvent& e ) 
{
    SetFocus();  //to regain/recapture keypress events
    if (isLoaded(1)) 
	{
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnMiddleUp ( wxMouseEvent& e ) {
    if (isLoaded(1)) {
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnLeftDown ( wxMouseEvent& e ) 
{
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnLeftUp ( wxMouseEvent& e ) {
    cout << "OnLeftUp" << endl;
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnPaint ( wxPaintEvent& e ) 
{
    wxMemoryDC  m;
    int         w, h;
    GetSize( &w, &h );
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::paint ( wxDC* dc ) 
{
	int  w, h;	
	dc->GetSize( &w, &h );
...
//----------------------------------------------------------------------
void KinematicsIntraCanvas::OnSize ( wxSizeEvent& e ) {
    //called when the size of the window/viewing area changes
}
//----------------------------------------------------------------------
bool KinematicsIntraCanvas::isLoaded ( const int which ) const {
    if (which==0) {
...
int KinematicsIntraCanvas::SaveProfile ( unsigned char* cFilename )
{
	SaveIntraPlan(surf, (char*)cFilename, m_timeInstances, m_nRefFrame);
	return 1;
}

bool KinematicsIntraCanvas::getKinematicsIntraDone (void) const
{
	return m_bKinematicsIntraDone;
}
//----------------------------------------------------------------------

//----------------------------------------------------------------------
IMPLEMENT_DYNAMIC_CLASS ( KinematicsIntraCanvas, wxPanel )
BEGIN_EVENT_TABLE       ( KinematicsIntraCanvas, wxPanel )
    EVT_PAINT(            KinematicsIntraCanvas::OnPaint        )
    EVT_ERASE_BACKGROUND( MainCanvas::OnEraseBackground )
    EVT_MOTION(           KinematicsIntraCanvas::OnMouseMove    )
    EVT_SIZE(             MainCanvas::OnSize         )
    EVT_LEFT_DOWN(        KinematicsIntraCanvas::OnLeftDown     )
    EVT_LEFT_UP(          KinematicsIntraCanvas::OnLeftUp       )
    EVT_MIDDLE_DOWN(      KinematicsIntraCanvas::OnMiddleDown   )
    EVT_MIDDLE_UP(        KinematicsIntraCanvas::OnMiddleUp     )
    EVT_RIGHT_DOWN(       KinematicsIntraCanvas::OnRightDown    )
    EVT_RIGHT_UP(         KinematicsIntraCanvas::OnRightUp      )
    EVT_CHAR(             KinematicsIntraCanvas::OnChar         )
END_EVENT_TABLE()
//======================================================================
