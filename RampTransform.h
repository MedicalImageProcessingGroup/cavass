...
// RampTransform
//----------------------------------------------------------------------
#ifndef __RampTransform_h
#define __RampTransform_h

#include  "GrayScaleTransform.h"

class RampTransform : public GrayScaleTransform {
  private:
    double            m_slope;
    ...
    inline double ramp ( const double x, const int max, const int min ) const {
  ...
  public:
    RampTransform ( MontageFrame* parent=NULL );
    void init ( void );
  ...
  private:
    DECLARE_DYNAMIC_CLASS(RampTransform)
    DECLARE_EVENT_TABLE()
};
