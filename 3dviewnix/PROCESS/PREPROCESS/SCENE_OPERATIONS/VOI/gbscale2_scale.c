...
/* Program to perform the intensity scaling
   of a scene, given the parameter file
   using gb-scale.
   by Anant Madabhushi
   Created on 06.01.04 */
...
void compute_scale();
int compute_scale_image(char *o_file, double a, int b, char *d_file);
...
/*****************************************************************************/
char *datafile[FEATURES], *param_file[FEATURES], *meanfile[FEATURES],
  *endname1[FEATURES], *endname2[FEATURES], *outfile[FEATURES];
int Way=0;
...
void usage()
{
  printf("Usage: gbscale2_scale <ori_file> <std_file> <param_file> ");
  printf("-radius <i> -hist_thresh <f> -num_regions <n>\n");
  exit(1);
}
...
char *argv[];
{
  ...
  args_parsed++;
  param_file[0] = argv[args_parsed];

  if (argc > 4)
    args_parsed++;

  if (strcmp(argv[args_parsed], "-radius") == 0)
    {
      args_parsed++;
      sscanf(argv[args_parsed], "%d", &SCALE_THRESH);
      args_parsed++;
    }

  if (strcmp(argv[args_parsed], "-hist_thresh") == 0)
    {
      args_parsed++;
      sscanf(argv[args_parsed], "%f", &HIST_THRESHOLD);
      args_parsed++;
    }

  if (strcmp(argv[args_parsed], "-num_regions") == 0)
    {
      args_parsed++;
     ...
     char *argv[];
{
  ...
  temp=compute_scale_image(datafile[0],1.0,0, outfile[0]);
 ...
 * PARAMETERS: original image, scale image, constant, destination file
 *     
 * SIDE EFFECTS: None
 ...
 *    Modified: 2/25/05 unsigned values cast to int for subtraction
 *           by Dewey Odhner.
 *    Modified: 9/26/06 bound checked in median calculation by Dewey Odhner.
 *
 *******************************************************************************************/
int compute_scale_image(char *o_file, double a, int b, char *d_file)
{
  ...
  char group[6],element[6], *datafile[FEATURES];
  ...
  int num_of_bytes, temp_int, median_int;
  unsigned short *std_scene;
 ...
 /* unsigned long  LARGEST_REGION[NUM_REGIONS_PAINT], SIZE_REGION[NUM_REGIONS_PAINT];
  ...
  std_scene = (unsigned short *) malloc(volume_size * sizeof(unsigned short));
  ...
  fp_in = fopen(param_file[0], "rb");
  if (fscanf(fp_in,"%d",&mean_med) != 1)
  {
    fprintf(stderr, "Cannot read parameter.\n");
    exit(1);
  }
  fclose(fp_in);
  ...
  */
  ...
  for(num_largest_regions = 1; num_largest_regions<(NUM_REGIONS_PAINT+1); num_largest_regions++)
      ...
      for (i = 1; i < count; i++)
	{
	  ...
	  while(j>0 && (int)int_que[j-1] > temp_int) 
	    {
      ...
      /* ---------------------------------------------------- */
      median_int = (! (count%2)) ? (int_que[count/2 - 1] + int_que[count/2])/2 : int_que[(count - 1)/2];
  ...
  /* ---------------------------------------------------------- */
  /*     Intensity Transformation takes place here.             */
  
  for(aaa=0;(int)aaa<volume_size;aaa++)
    {
      if (num_of_bits == 8)
	pppi1 = data_scale8[aaa];
      else
	pppi1 = data_scale16[aaa];

      if ((int)pppi1 <= median_int)
	{
	  std_scene[aaa] = (unsigned short)((float) mean_med +  ( (float) mean_med/ (float) median_int)* ((float) pppi1 - (float) median_int));
	}
      else if (((int)pppi1 > median_int) && (pppi1 <= max_percentile_int))
	{
	  std_scene[aaa]=(unsigned short)((( (float) (4095 - mean_med)/ ( (float)max_percentile_int - median_int))*( (float) pppi1 - (float) median_int)) + (float) mean_med);
	}
      else 
	std_scene[aaa] = 4095;
    }
  ...
  if (num_of_bits == 8)
    {
	  ...
	  for (j=0; j<volume_size; j++)
	    ((unsigned char *)std_scene)[j] = (unsigned char)std_scene[j];
    }
  else
    {
  ...
  if ((error_code && error_code!=106 && error_code!=107) ||
      VWriteData((char *)std_scene,num_of_bytes,volume_size,fp_out,&j))
  {
    fprintf(stderr, "Failure writing output.\n");
	exit(-1);
  }
		...
		for (i = 0; i < sphere_no_points[k]; i++) 
		  {
		    ...
		    for(j=0;j<FEATURES;j++)
		      {
			temp[j] = abs( data_n8[j][z * slice_size + y * pcol + x] - mean[j]);		
		...
		for (i = 0; i < sphere_no_points[k]; i++) 
		  {
		    ...
		    for(j=0;j<FEATURES;j++)
		      {
			temp[j] = abs( data_n16[j][z * slice_size + y * pcol + x] - mean[j]);		
			tti1 = tti1+(temp[j])*pow_value[j];
			if(temp[j]>diff_value_max[j])
			  edge_flag = 1;
		      }
