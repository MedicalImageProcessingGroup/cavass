...
/* Program to compute the median intensity 
   of the largest gb-scale region.
   The median intensity computed by this program
   is used by "gbscale2_train" to compute
   the average median intensity over all 
   the training files.
   by Anant Madabhushi
   Created on 06.01.04 */
...
void usage()
{
  fprintf(stderr, "Usage: gbscale2_std <ori_file> -radius <i>");
  fprintf(stderr, " -hist_thresh <f> -num_regions <n>\n");
  exit(1);
}
     ...
     char *argv[];
{
  ...
  temp=compute_scale_image(datafile[0],1.0,0);
  ...
  for(num_largest_regions = 1; num_largest_regions<(NUM_REGIONS_PAINT+1); num_largest_regions++)
    {
	  ...
	  gbs_fp = fopen("C:\\gbscale01.txt", "wt");
	  if( gbs_fp == NULL )
		  return -1;
	  fprintf(gbs_fp, "%d", (int)SCALED_MED_REGION[num_largest_regions - 1]);
	  fclose(gbs_fp);
