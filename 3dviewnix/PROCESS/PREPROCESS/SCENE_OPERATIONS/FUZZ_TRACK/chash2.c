 ...
 *****************************************************************************/
Chash*
chash_create2(long hashsize, int itemsize, unsigned short (*val)(void *), int (*idcmp)(void *, void *))
{
  Chash *H;
  long s;
...
void chash_repush2(Chash *H, VoxelWithValue *v, unsigned short wo)
{
  ...
  if (H->idcmp)
    ...
    for (t=H->hh[wo];
	    v->x!=((VoxelChashElem*)H->q)[t].c.x||
		v->y!=((VoxelChashElem*)H->q)[t].c.y||
		v->z!=((VoxelChashElem*)H->q)[t].c.z;
