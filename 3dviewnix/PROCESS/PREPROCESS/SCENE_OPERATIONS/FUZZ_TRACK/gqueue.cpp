/*
  ...
  Q->L.color[i]: the color of i (WHITE=never inserted, GRAY=inserted,
  BLACK=removed)
...
GQueue *CreateGQueue(int nbuckets, int nelems, int *value)
{
    ...
    if (Q != NULL)
    {
        ...
        if ( (Q->C.first != NULL) && (Q->C.last != NULL) )
        {
            ...
            Q->L.next = (int *)malloc(nelems*sizeof(int));
            Q->L.prev = (int *)malloc(nelems*sizeof(int));
            Q->L.color = (char *)malloc(nelems);
            Q->L.nelems = nelems;
            Q->L.value   = value;
            if (Q->L.next != NULL && Q->L.prev!= NULL && Q->L.color != NULL)
            {
                ResetGQueue(Q);
            }
            else
...
void DestroyGQueue(GQueue **Q)
{
    ...
    if (aux != NULL)
    {
        ...
        if (aux->L.next  != NULL) free(aux->L.next);
        if (aux->L.prev  != NULL) free(aux->L.prev);
        if (aux->L.color != NULL) free(aux->L.color);
        free(aux);
        *Q = NULL;
    }
}
