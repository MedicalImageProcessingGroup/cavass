 ...
 *****************************************************************************/
Hheap*
hheap_create(long hashsize, int itemsize, long (*hf)(long hashsize, void *v), int (*valcmp)(void *v, void *vv), int (*idcmp)(void *v, void *vv))
{
  Hheap *H;
  long s;
...
int hheap_push(Hheap *H, void *v)
{
  ...
  n = ++H->size;
  p = n / 2;
  while (n > 1L && Value_Cmp(HQElem(H->h[p]).c, v) < 0)
  {
    H->h[n] = H->h[p];
    HQElem(H->h[p]).i = n;
    n = p;
...
void hheap_pop(Hheap *H, void *v)
{
  ...
  if (s > 2L)
  {
    ...
    while (m != n)
    {
      ...
      l = n + n;
      r = l + 1;
      if (l <= H->size && Value_Cmp(HQElem(H->h[l]).c, w) > 0)
      {
        m = l;
        wm = (void *)HQElem(H->h[l]).c;
      }
      else
      {
        m = n;
      ...
      if (r <= H->size && Value_Cmp(HQElem(H->h[r]).c, wm) > 0)
        m = r;
      if (m != n)
      {
        H->h[n] = H->h[m];
