...
% cc gradient.c -o gradient  -I$VIEWNIX_ENV/INCLUDE $VIEWNIX_ENV/LIBRARY/lib3dviewnix.a -lX11 -lm
...
char *argv[];
{
	...
	{
		printf("Usage:\n");
		printf("%% gradient input output mode\n");
		printf("where:\n");
	...
	for(i=0; i<sl.slices[j]; i++, k++)
	{
		...
		if(vh.scn.num_of_bits == 8)
		{
			...
			/* Filter */
			gradient8(in_buffer8, width, height, out_buffer8, MIN, MAX);
		...
		else
		{
			...
			/* Filter */
			gradient16(in_buffer16, width, height, out_buffer16, MIN, MAX);
