...
% cc toboggan.c -o toboggan  $VIEWNIX_ENV/INCLUDE $VIEWNIX_ENV/LIBRARY/3dviewnix.a -lX11 -lm
...
char *argv[];
{
	...
	unsigned char *in_buffer8, *out_buffer8, *temp8;
	unsigned short *in_buffer16, *out_buffer16, *temp16;


	if(argc<4)
	{
		printf("Usage:\n");
		printf("%% toboggan input output mode\n");
		printf("where:\n");
	...
	if(vh.scn.num_of_bits == 8)
	{
    	...
    	/* create buffer for one grey image */
    	if( (temp8 = (unsigned char *) calloc(1, length) ) == NULL)
    	{
       		printf("ERROR: Can't allocate input image buffer.\n");
       		exit(1);
    	}
	...
	else
	{
    	...
    	/* create buffer for one grey image */
    	if( (temp16 = (unsigned short *) calloc(1, length) ) == NULL)
    	{
       		printf("ERROR: Can't allocate input image buffer.\n");
       		exit(1);
    	}
	...
	for(i=0; i<sl.slices[j]; i++, k++)
	{
		...
		if(vh.scn.num_of_bits == 8)
		{
			...
			/* Filter */
			dce8(in_buffer8, width, height, temp8);
            toboggan8(in_buffer8, temp8, width, height, out_buffer8);
		...
		else
		{
			...
			/* Filter */
			dce16(in_buffer16, width, height, temp16);
            toboggan16(in_buffer16, temp16, width, height, out_buffer16);
