...
% cc gaussian2d.c -o gaussian2d -I/usr/chromos2_3/3dviewnix/INCLUDE $VIEWNIX_ENV/LIBRARY/3dviewnix.a -lX11 -lm
...
char *argv[];
{
	...
	{
		printf("Usage:\n");
		printf("%% gaussian2d input output mode sigma\n");
		printf("where:\n");
	...
	for(i=0; i<sl.slices[j]; i++, k++)
	{
		...
		if(vh.scn.num_of_bits == 8)
		{
			...
			/* Filter */
			gaussian8(in_buffer8b, width, height, out_buffer8, sigma);
		...
		else
		{
			...
			/* Filter */
			gaussian16(in_buffer16, width, height, out_buffer16, sigma);
