...
//======================================================================
/**
 * \file   ShowScreenCanvas.cpp
 * \brief  ShowScreenCanvas class implementation
...
using namespace std;
//----------------------------------------------------------------------
const int  ShowScreenCanvas::sSpacing=1;  ///< space, in pixels, between each slice (on the screen in the frame in the canvas)
//----------------------------------------------------------------------
/** \brief ShowScreenCanvas ctor. */
ShowScreenCanvas::ShowScreenCanvas ( void ) {
    init();
}
//----------------------------------------------------------------------
/** \brief ShowScreenCanvas ctor. */
ShowScreenCanvas::ShowScreenCanvas ( wxWindow* parent, MainFrame* parent_frame,
...
void ShowScreenCanvas::init ( void ) {
    mFileOrDataCount = 0;
    mScale           = 1.0;
    mOverlay         = false;
    mRows            = 0;
...
//----------------------------------------------------------------------
/** \brief ShowScreenCanvas dtor. */
ShowScreenCanvas::~ShowScreenCanvas ( void ) {
    cout << "ShowScreenCanvas::~ShowScreenCanvas" << endl;
    wxLogMessage( "ShowScreenCanvas::~ShowScreenCanvas" );
    release();
}
//----------------------------------------------------------------------
/** \brief release memory allocated to this object. */
void ShowScreenCanvas::release ( void ) {
...
void ShowScreenCanvas::loadData ( char* name,
 ...
 */
void ShowScreenCanvas::loadFile ( const char* const fn ) {
    SetCursor( wxCursor(wxCURSOR_WAIT) );    wxYield();
    release();
    ...
    assert( mFileOrDataCount==0 );
    CavassData*  cd = new CavassData( fn, false, false );
	//ChunkData*  cd = new ChunkData( fn, ChunkData::defaultSlicesPerChunk,
	//	ChunkData::defaultOverlapSliceCount, false );
    assert( mCavassData==NULL );
    mCavassData = cd;
    mFileOrDataCount = 1;
 ...
 */
void ShowScreenCanvas::initLUT ( const int which ) {
 ...
 *  (of type wxBitmap)
 */
void ShowScreenCanvas::freeImagesAndBitmaps ( void ) {
    if (mImages!=NULL) {
...
void ShowScreenCanvas::reload ( void ) {
...
void ShowScreenCanvas::OnChar ( wxKeyEvent& e ) {
    //cout << "ShowScreenCanvas::OnChar" << endl;
    wxLogMessage( "ShowScreenCanvas::OnChar" );
    if (e.m_keyCode==' ') {
        if (isLoaded(1)) {
...
void ShowScreenCanvas::OnMouseMove ( wxMouseEvent& e ) {
...
void ShowScreenCanvas::OnRightDown ( wxMouseEvent& e ) {
    cout << "ShowScreenCanvas::OnRightDown" << endl;
    wxLogMessage("ShowScreenCanvas::OnRightDown");
    SetFocus();  //to regain/recapture keypress events
    //pass the event up to the parent frame
    ShowScreenFrame*  p = (ShowScreenFrame*)m_parent_frame;
    wxCommandEvent  unused;
    //simulate the middle button for a two button mouse
    if (e.AltDown())    cout << "alt down" << endl;
    if (e.ControlDown())    cout << "ctrl down" << endl;
    if (e.ShiftDown())    cout << "shift down" << endl;
...
void ShowScreenCanvas::OnRightUp ( wxMouseEvent& e ) {
}
//----------------------------------------------------------------------
/** \brief Callback to handle middle mouse button down events. */
void ShowScreenCanvas::OnMiddleDown ( wxMouseEvent& e ) {
    cout << "ShowScreen::OnMiddleDown" << endl;
    wxLogMessage("ShowScreen::OnMiddleDown");
    SetFocus();  //to regain/recapture keypress events
    //pass the event up to the parent frame
    ShowScreenFrame*  p = (ShowScreenFrame*)m_parent_frame;
    wxCommandEvent  unused;
...
void ShowScreenCanvas::OnMiddleUp ( wxMouseEvent& e ) {
}
//----------------------------------------------------------------------
/** \brief Callback to handle left mouse button down events. */
void ShowScreenCanvas::OnLeftDown ( wxMouseEvent& e ) {
...
void ShowScreenCanvas::OnLeftUp ( wxMouseEvent& e ) {
    cout << "OnLeftUp" << endl;
...
void ShowScreenCanvas::OnPaint ( wxPaintEvent& e ) {
    wxMemoryDC  m;
    int         w, h;
    GetSize( &w, &h );
    wxBitmap    bitmap(w, h);
...
void ShowScreenCanvas::paint ( wxDC* dc ) {
 ...
 */
bool ShowScreenCanvas::isLoaded ( const int which ) const {
    if (which==0) {
 ...
 */
int ShowScreenCanvas::getCenter ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.m_center;
 ...
 */
int ShowScreenCanvas::getMax ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.m_max;
 ...
 */
int ShowScreenCanvas::getMin ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.m_min;
 ...
 */
int ShowScreenCanvas::getNoSlices ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.m_zSize;
 ...
 */
bool ShowScreenCanvas::getOverlay ( void ) const {
    return mOverlay;
}
//----------------------------------------------------------------------
 ...
 */
double ShowScreenCanvas::getScale ( void ) const {
    return mScale;
}
//----------------------------------------------------------------------
 ...
 */
int ShowScreenCanvas::getSliceNo ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.m_sliceNo;
 ...
 */
int ShowScreenCanvas::getWidth ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.m_width;
 ...
 */
bool ShowScreenCanvas::getInvert ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.mInvert;
 ...
 */
double ShowScreenCanvas::getB ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.getB();
 ...
 */
double ShowScreenCanvas::getG ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.getG();
 ...
 */
double ShowScreenCanvas::getR ( const int which ) const {
    if (which==0) {
        assert( mCavassData!=NULL );
        const CavassData&  cd = *mCavassData;
        return cd.getR();
 ...
 */
void ShowScreenCanvas::setB ( const int which, const double b ) {
    if (which==0) {
        assert( mCavassData!=NULL );
 ...
 */
void ShowScreenCanvas::setCenter ( const int which, const int center ) {
    if (which==0) {
        assert( mCavassData!=NULL );
        CavassData&  cd = *mCavassData;
        cd.m_center = center;
 ...
 */
void ShowScreenCanvas::setG ( const int which, const double g ) {
    if (which==0) {
        assert( mCavassData!=NULL );
        CavassData&  cd = *mCavassData;
        cd.setG( g );
 ...
 */
void ShowScreenCanvas::setInvert ( const int which, const bool invert ) {
    if (which==0) {
        assert( mCavassData!=NULL );
        CavassData&  cd = *mCavassData;
        cd.mInvert = invert;
 ...
 */
void ShowScreenCanvas::setOverlay ( const bool overlay ) { 
    mOverlay = overlay;
}
//----------------------------------------------------------------------
 ...
 */
void ShowScreenCanvas::setR ( const int which, const double r ) {
    if (which==0) {
        assert( mCavassData!=NULL );
        CavassData&  cd = *mCavassData;
        cd.setR( r );
 ...
 */
void ShowScreenCanvas::setScale   ( const double scale )  {
 ...
 */
void ShowScreenCanvas::setSliceNo ( const int which, int sliceNo ) {
 ...
 */
void ShowScreenCanvas::setWidth ( const int which, const int width ) {
    if (which==0) {
        assert( mCavassData!=NULL );
        CavassData&  cd = *mCavassData;
        cd.m_width = width;
...
void ShowScreenCanvas::OnMouseWheel ( wxMouseEvent& e ) {
    ShowScreenFrame*  ef = dynamic_cast<ShowScreenFrame*>(m_parent_frame);
    ef->OnMouseWheel(e);
}
//----------------------------------------------------------------------
IMPLEMENT_DYNAMIC_CLASS ( ShowScreenCanvas, wxPanel )
BEGIN_EVENT_TABLE       ( ShowScreenCanvas, wxPanel )
    EVT_CHAR(             ShowScreenCanvas::OnChar         )
    EVT_ERASE_BACKGROUND( MainCanvas::OnEraseBackground )
    EVT_LEFT_DOWN(        ShowScreenCanvas::OnLeftDown     )
    EVT_LEFT_UP(          ShowScreenCanvas::OnLeftUp       )
    EVT_MIDDLE_DOWN(      ShowScreenCanvas::OnMiddleDown   )
    EVT_MIDDLE_UP(        ShowScreenCanvas::OnMiddleUp     )
    EVT_MOTION(           ShowScreenCanvas::OnMouseMove    )
    EVT_MOUSEWHEEL(       ShowScreenCanvas::OnMouseWheel   )
    EVT_PAINT(            ShowScreenCanvas::OnPaint        )
    EVT_RIGHT_DOWN(       ShowScreenCanvas::OnRightDown    )
    EVT_RIGHT_UP(         ShowScreenCanvas::OnRightUp      )
