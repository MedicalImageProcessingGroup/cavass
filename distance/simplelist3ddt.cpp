...
static void usage ( char* programName ) {
  std::cerr << "Usage:" << std::endl << std::endl
      << "    " << programName << " [-d] [-6|18|26] inputImageFile outputDistanceMapImageFile" << std::endl
      << "        -d = output distance map values as doubles" << std::endl
      << "             (default is " << ::outputDouble << ")" << std::endl
      << "        -s = produce symmetric distance transform"  << std::endl
      << "             (default is " << ::symmetric << ")"    << std::endl
	  << "        -6|18|26 = 6, 18, or 26 connected (default is " << ::connectivity << ")" << std::endl
      << std::endl;
  exit( EXIT_FAILURE );
}
//----------------------------------------------------------------------
int main ( int argc, char* argv[] ) {
    ...
    for ( ; ; ) {
        ...
        if (strcmp(argv[nextArg],"-d")==0) {
            ...
            ::symmetric = true;
            ++nextArg;
        } else if (strcmp(argv[nextArg],"-6")==0) {
			cout << "-6" << endl;
			::connectivity = DistanceTransform3D::Face6;
            ++nextArg;
        } else if (strcmp(argv[nextArg],"-18")==0) {
			cout << "-18" << endl;
			::connectivity = DistanceTransform3D::Edge18;
            ++nextArg;
        } else if (strcmp(argv[nextArg],"-26")==0) {
			cout << "-26" << endl;
			::connectivity = DistanceTransform3D::Vertex26;
            ++nextArg;
    ...
    SimpleList3D  dt( size[0], size[1], size[2] );
    dt.setSymmetricFlag( ::symmetric );
	dt.mConnectivity = ::connectivity;
    dt.doTransform( d );
